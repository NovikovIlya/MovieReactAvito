...
import { Empty, Pagination, Select, Spin, Popover } from "antd";
...
const New = () => {
  ...
  const {num} = useAppSelector((state) => state.sliceMovie);
  const {
    data,
    isFetching,
    error,
  ...
  return (
    <>
      ...
      {isLoadingAuth ? (
        <div className={styles.zagr100}>Загружаем данные пользователя...</div>
      ) : (
          ...
          <div className={styles.parent}>
            {dataPopular?.docs?.map((item) => {
              return (
                <div
                  key={item.id}
                  className="mda1 zz rowChild f-flex justify-content-start"
                >
                  <div className={styles.text}>
                    <div className="toH">{item.name}</div>
                  </div>
                  {item.poster.url ? (
                    <Link to={`/${item.id}`}>
                      <img
